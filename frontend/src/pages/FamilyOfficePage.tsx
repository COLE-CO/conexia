    ...
    <div className="p-8 min-h-screen bg-neutral-bg relative overflow-hidden">
      ...
      <div className="flex items-center gap-1 mb-6 border border-neutral-border rounded-xl p-1 w-fit bg-neutral-surface shadow-sm animate-fade-in-up animation-delay-160">
        {(['balances', 'vencimientos', 'cartera'] as Tab[]).map(tab => (
          ...
          >
            {tab === 'balances' && (
              <span className="inline-flex items-center gap-1.5"><FileSpreadsheet size={15} /> Balances</span>
            )}
            {tab === 'vencimientos' && (
              <span className="inline-flex items-center gap-1.5"><CalendarClock size={15} /> Vencimientos</span>
            )}
            {tab === 'cartera' && (
              <span className="inline-flex items-center gap-1.5"><Users size={15} /> Cartera</span>
            )}
          </button>
        ))}
      </div>
